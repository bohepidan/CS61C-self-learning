...
void transpose_blocking(int n, int blocksize, int *dst, int *src) {
    // YOUR CODE HERE
    int blockinrow = n/blocksize;
    int totalblock = (n/blocksize)*(n/blocksize);
    for(int i=0; i<totalblock; i++){      //block loop
        int * subsrc = src+ (i/blockinrow) * (n*blocksize) + (i%blockinrow) * blocksize;
        int * subdst = dst+ (i/blockinrow) * blocksize + (i%blockinrow) * (n*blocksize);
        for(int x=0; x<blocksize; x++){
            for (int y=0; y < blocksize; y++){
                subdst[x + y * n] = subsrc[y + x * n];
            }
        }
    }
}
